...
# Метод .weekday() считает, что неделя начинается с нуля, метод .isoweekday(), что с единицы.
...
# компоненты. Предварительно строку нужно преобразовать. Для этого есть метод .strptime().
...
# то в Питоне мы можем их сравнивать, как если бы мы сравнивали объекты datetime
